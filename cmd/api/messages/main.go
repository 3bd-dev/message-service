...
import (
	...
	"github.com/3bd-dev/wallet-service/internal/handlers/messageapi"
...
func main() {
	log := logger.New(os.Stdout, logger.LevelInfo, "MESSAGES-API")
...
func run(ctx context.Context, log *logger.Logger) error {
	...
	messageReviewRepo := postgres.NewMessageReviewRepo(db)

	// Wallet service setup
	messageService := message.NewService(log, messageRepo, userRepo, messageReviewRepo, db)
	...
	messageapi.Routes(httpmux, messageapi.Config{
